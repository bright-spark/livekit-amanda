...
def get_redis():
    if not redis_available or not REDIS_URL:
        return None
    return redis_pkg.Redis.from_url(REDIS_URL, decode_responses=True, ssl_cert_reqs=None)

async def get_history(user_id: str) -> List[Dict]:
    if USE_REDIS and redis_available:
        try:
            redis = get_redis()
            data = redis.get(f"history:{user_id}")
            if data:
                import json
...
async def set_history(user_id: str, history: List[Dict]):
    if USE_REDIS and redis_available:
        try:
            redis = get_redis()
            import json
            redis.set(f"history:{user_id}", json.dumps(history), ex=REDIS_HISTORY_EXPIRE_SECONDS)
            return
